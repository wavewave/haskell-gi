...
newtype RangeAccessible = RangeAccessible (SP.ManagedPtr RangeAccessible)
    deriving (Eq)

instance SP.ManagedPtrNewtype RangeAccessible where
    toManagedPtr (RangeAccessible p) = p

foreign import ccall "gtk_range_accessible_get_type"
    c_gtk_range_accessible_get_type :: IO B.Types.GType

instance B.Types.TypedObject RangeAccessible where
    glibType = c_gtk_range_accessible_get_type

instance B.Types.GObject RangeAccessible

-- | Type class for types which can be safely cast to `RangeAccessible`, for instance with `toRangeAccessible`.
class (SP.GObject o, O.IsDescendantOf RangeAccessible o) => IsRangeAccessible o
instance (SP.GObject o, O.IsDescendantOf RangeAccessible o) => IsRangeAccessible o
...
toRangeAccessible :: (MIO.MonadIO m, IsRangeAccessible o) => o -> m RangeAccessible
toRangeAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo RangeAccessible

-- | Convert 'RangeAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe RangeAccessible) where
    gvalueGType_ = c_gtk_range_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr RangeAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr RangeAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject RangeAccessible ptr
...
instance (info ~ ResolveRangeAccessibleMethod t RangeAccessible, O.OverloadedMethod info RangeAccessible p) => OL.IsLabel t (RangeAccessible -> p) where
...
instance (info ~ ResolveRangeAccessibleMethod t RangeAccessible, O.OverloadedMethod info RangeAccessible p, R.HasField t RangeAccessible p) => R.HasField t RangeAccessible p where
    getField = O.overloadedMethod @info

#endif

instance (info ~ ResolveRangeAccessibleMethod t RangeAccessible, O.OverloadedMethodInfo info RangeAccessible) => OL.IsLabel t (O.MethodProxy info RangeAccessible) where
#if MIN_VERSION_base(4,10,0)
