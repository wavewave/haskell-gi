...
instance SP.ManagedPtrNewtype ScrolledWindowAccessible where
    toManagedPtr (ScrolledWindowAccessible p) = p
...
instance B.Types.TypedObject ScrolledWindowAccessible where
    glibType = c_gtk_scrolled_window_accessible_get_type
...
toScrolledWindowAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo ScrolledWindowAccessible

-- | Convert 'ScrolledWindowAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe ScrolledWindowAccessible) where
    gvalueGType_ = c_gtk_scrolled_window_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr ScrolledWindowAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr ScrolledWindowAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject ScrolledWindowAccessible ptr
