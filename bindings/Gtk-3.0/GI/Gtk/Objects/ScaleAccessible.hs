...
newtype ScaleAccessible = ScaleAccessible (SP.ManagedPtr ScaleAccessible)
    deriving (Eq)

instance SP.ManagedPtrNewtype ScaleAccessible where
    toManagedPtr (ScaleAccessible p) = p

foreign import ccall "gtk_scale_accessible_get_type"
    c_gtk_scale_accessible_get_type :: IO B.Types.GType

instance B.Types.TypedObject ScaleAccessible where
    glibType = c_gtk_scale_accessible_get_type

instance B.Types.GObject ScaleAccessible

-- | Type class for types which can be safely cast to `ScaleAccessible`, for instance with `toScaleAccessible`.
class (SP.GObject o, O.IsDescendantOf ScaleAccessible o) => IsScaleAccessible o
instance (SP.GObject o, O.IsDescendantOf ScaleAccessible o) => IsScaleAccessible o
...
toScaleAccessible :: (MIO.MonadIO m, IsScaleAccessible o) => o -> m ScaleAccessible
toScaleAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo ScaleAccessible

-- | Convert 'ScaleAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe ScaleAccessible) where
    gvalueGType_ = c_gtk_scale_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr ScaleAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr ScaleAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject ScaleAccessible ptr
...
instance (info ~ ResolveScaleAccessibleMethod t ScaleAccessible, O.OverloadedMethod info ScaleAccessible p) => OL.IsLabel t (ScaleAccessible -> p) where
...
instance (info ~ ResolveScaleAccessibleMethod t ScaleAccessible, O.OverloadedMethod info ScaleAccessible p, R.HasField t ScaleAccessible p) => R.HasField t ScaleAccessible p where
    getField = O.overloadedMethod @info

#endif

instance (info ~ ResolveScaleAccessibleMethod t ScaleAccessible, O.OverloadedMethodInfo info ScaleAccessible) => OL.IsLabel t (O.MethodProxy info ScaleAccessible) where
#if MIN_VERSION_base(4,10,0)
...
type ScaleAccessibleSignalList = ('[ '("activeDescendantChanged", Atk.Object.ObjectActiveDescendantChangedSignalInfo), '("announcement", Atk.Object.ObjectAnnouncementSignalInfo), '("boundsChanged", Atk.Component.ComponentBoundsChangedSignalInfo), '("childrenChanged", Atk.Object.ObjectChildrenChangedSignalInfo), '("focusEvent", Atk.Object.ObjectFocusEventSignalInfo), '("notify", GObject.Object.ObjectNotifySignalInfo), '("propertyChange", Atk.Object.ObjectPropertyChangeSignalInfo), '("stateChange", Atk.Object.ObjectStateChangeSignalInfo), '("valueChanged", Atk.Value.ValueValueChangedSignalInfo), '("visibleDataChanged", Atk.Object.ObjectVisibleDataChangedSignalInfo)] :: [(Symbol, *)])
