...
instance SP.ManagedPtrNewtype SocketAccessible where
    toManagedPtr (SocketAccessible p) = p
...
instance B.Types.TypedObject SocketAccessible where
    glibType = c_gtk_socket_accessible_get_type

instance B.Types.GObject SocketAccessible

-- | Type class for types which can be safely cast to `SocketAccessible`, for instance with `toSocketAccessible`.
...
-- | Cast to `SocketAccessible`, for types for which this is known to be safe. For general casts, use `Data.GI.Base.ManagedPtr.castTo`.
toSocketAccessible :: (MIO.MonadIO m, IsSocketAccessible o) => o -> m SocketAccessible
toSocketAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo SocketAccessible

-- | Convert 'SocketAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe SocketAccessible) where
    gvalueGType_ = c_gtk_socket_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr SocketAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr SocketAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject SocketAccessible ptr
...
-- method SocketAccessible::embed
-- method type : OrdinaryMethod
-- Args: [ Arg
--           { argCName = "socket"
--           , argType =
--               TInterface Name { namespace = "Gtk" , name = "SocketAccessible" }
...
foreign import ccall "gtk_socket_accessible_embed" gtk_socket_accessible_embed :: 
    Ptr SocketAccessible ->                 -- socket : TInterface (Name {namespace = "Gtk", name = "SocketAccessible"})
...
socketAccessibleEmbed ::
    ...
    -> T.Text
    -> m ()
socketAccessibleEmbed socket path = liftIO $ do
    socket' <- unsafeManagedPtrCastPtr socket
    path' <- textToCString path
    gtk_socket_accessible_embed socket' path'
...
instance O.OverloadedMethodInfo SocketAccessibleEmbedMethodInfo a where
    overloadedMethodInfo = P.Just (O.ResolvedSymbolInfo {
        O.resolvedSymbolName = "GI.Gtk.Objects.SocketAccessible.socketAccessibleEmbed",
        O.resolvedSymbolURL = "https://hackage.haskell.org/package/gi-gtk-3.0.41/docs/GI-Gtk-Objects-SocketAccessible.html#v:socketAccessibleEmbed"
        })
