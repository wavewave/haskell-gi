...
newtype MenuItemAccessible = MenuItemAccessible (SP.ManagedPtr MenuItemAccessible)
    deriving (Eq)

instance SP.ManagedPtrNewtype MenuItemAccessible where
    toManagedPtr (MenuItemAccessible p) = p

foreign import ccall "gtk_menu_item_accessible_get_type"
    c_gtk_menu_item_accessible_get_type :: IO B.Types.GType

instance B.Types.TypedObject MenuItemAccessible where
    glibType = c_gtk_menu_item_accessible_get_type

instance B.Types.GObject MenuItemAccessible

-- | Type class for types which can be safely cast to `MenuItemAccessible`, for instance with `toMenuItemAccessible`.
class (SP.GObject o, O.IsDescendantOf MenuItemAccessible o) => IsMenuItemAccessible o
instance (SP.GObject o, O.IsDescendantOf MenuItemAccessible o) => IsMenuItemAccessible o
...
toMenuItemAccessible :: (MIO.MonadIO m, IsMenuItemAccessible o) => o -> m MenuItemAccessible
toMenuItemAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo MenuItemAccessible

-- | Convert 'MenuItemAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe MenuItemAccessible) where
    gvalueGType_ = c_gtk_menu_item_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr MenuItemAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr MenuItemAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject MenuItemAccessible ptr
...
instance (info ~ ResolveMenuItemAccessibleMethod t MenuItemAccessible, O.OverloadedMethod info MenuItemAccessible p) => OL.IsLabel t (MenuItemAccessible -> p) where
...
instance (info ~ ResolveMenuItemAccessibleMethod t MenuItemAccessible, O.OverloadedMethod info MenuItemAccessible p, R.HasField t MenuItemAccessible p) => R.HasField t MenuItemAccessible p where
    getField = O.overloadedMethod @info

#endif

instance (info ~ ResolveMenuItemAccessibleMethod t MenuItemAccessible, O.OverloadedMethodInfo info MenuItemAccessible) => OL.IsLabel t (O.MethodProxy info MenuItemAccessible) where
#if MIN_VERSION_base(4,10,0)
