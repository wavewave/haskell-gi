...
instance SP.ManagedPtrNewtype ExpanderAccessible where
    toManagedPtr (ExpanderAccessible p) = p
...
instance B.Types.TypedObject ExpanderAccessible where
    glibType = c_gtk_expander_accessible_get_type
...
toExpanderAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo ExpanderAccessible

-- | Convert 'ExpanderAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe ExpanderAccessible) where
    gvalueGType_ = c_gtk_expander_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr ExpanderAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr ExpanderAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject ExpanderAccessible ptr
