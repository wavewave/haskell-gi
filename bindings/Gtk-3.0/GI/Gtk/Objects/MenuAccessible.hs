...
newtype MenuAccessible = MenuAccessible (SP.ManagedPtr MenuAccessible)
    deriving (Eq)

instance SP.ManagedPtrNewtype MenuAccessible where
    toManagedPtr (MenuAccessible p) = p

foreign import ccall "gtk_menu_accessible_get_type"
    c_gtk_menu_accessible_get_type :: IO B.Types.GType

instance B.Types.TypedObject MenuAccessible where
    glibType = c_gtk_menu_accessible_get_type

instance B.Types.GObject MenuAccessible

-- | Type class for types which can be safely cast to `MenuAccessible`, for instance with `toMenuAccessible`.
class (SP.GObject o, O.IsDescendantOf MenuAccessible o) => IsMenuAccessible o
instance (SP.GObject o, O.IsDescendantOf MenuAccessible o) => IsMenuAccessible o
...
toMenuAccessible :: (MIO.MonadIO m, IsMenuAccessible o) => o -> m MenuAccessible
toMenuAccessible = MIO.liftIO . B.ManagedPtr.unsafeCastTo MenuAccessible

-- | Convert 'MenuAccessible' to and from 'Data.GI.Base.GValue.GValue'. See 'Data.GI.Base.GValue.toGValue' and 'Data.GI.Base.GValue.fromGValue'.
instance B.GValue.IsGValue (Maybe MenuAccessible) where
    gvalueGType_ = c_gtk_menu_accessible_get_type
    gvalueSet_ gv P.Nothing = B.GValue.set_object gv (FP.nullPtr :: FP.Ptr MenuAccessible)
    gvalueSet_ gv (P.Just obj) = B.ManagedPtr.withManagedPtr obj (B.GValue.set_object gv)
    gvalueGet_ gv = do
        ptr <- B.GValue.get_object gv :: IO (FP.Ptr MenuAccessible)
        if ptr /= FP.nullPtr
        then P.Just <$> B.ManagedPtr.newObject MenuAccessible ptr
...
instance (info ~ ResolveMenuAccessibleMethod t MenuAccessible, O.OverloadedMethod info MenuAccessible p) => OL.IsLabel t (MenuAccessible -> p) where
...
instance (info ~ ResolveMenuAccessibleMethod t MenuAccessible, O.OverloadedMethod info MenuAccessible p, R.HasField t MenuAccessible p) => R.HasField t MenuAccessible p where
    getField = O.overloadedMethod @info

#endif

instance (info ~ ResolveMenuAccessibleMethod t MenuAccessible, O.OverloadedMethodInfo info MenuAccessible) => OL.IsLabel t (O.MethodProxy info MenuAccessible) where
#if MIN_VERSION_base(4,10,0)
